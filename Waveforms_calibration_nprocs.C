...
void Waveforms_calibration_nprocs(
    TString source_path = "/media/doc/DATA/entanglement/pure_data/",
    TString file_name = "/media/doc/DATA/entanglement/pure_data/07a8de9a_20221206_094650.root",TString temp_DIR = "", TString postfix = "_0")
{
    ...
    for (Int_t num_int = 0; num_int < intervals; num_int++)
    {
        ...
        for (Long_t iEvent = Events_for_cuts_left; iEvent < Events_for_cuts_right; iEvent++)
        {
            ...
            for (Int_t channel_number = 0; channel_number < 32; channel_number++)
            {
                ...
                Float_t gateIntegral = channel_info[channel_number].Get_Charge(GATE_BEG, GATE_END);
                if (
                    gateIntegral > 4000
    ...
    ////////////////////////////////////
        ...
        for (Long_t iEvent = Events_for_cuts_left; iEvent < Events_for_cuts_right; iEvent++)
        {
            ...
            for (Int_t channel_number = 0; channel_number < 32; channel_number++)
            {
                ...
                //&& abs(channel_info[32].Get_time(GATE_BEG, GATE_END)-channel_info[33].Get_time(GATE_BEG, GATE_END)) < 3
                ...
                && channel_info[35].Get_Amplitude(GATE_BEG, GATE_END) < 400
        ...
        for (Long_t entry_num = Events_for_cuts_left;  entry_num< Events_for_cuts_right; entry_num++)
        {
            ...
            for (Int_t channel_number = 0; channel_number < total_channels; channel_number++)
            {
                ...
                if (channel_number < 34) 
                {
                    ...
                    if (channel_number == 33 || channel_number == 32) short_channel_info[channel_number].charge = (float)energy_in_peak/mean_int[channel_number]*channel_info[channel_number].Get_Charge(GATE_BEG, CH_GATE_END);
                    else short_channel_info[channel_number].charge = 
