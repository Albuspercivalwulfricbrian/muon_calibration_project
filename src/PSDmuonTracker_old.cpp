...
{
	fN_Hits = trackinfo->hits.size();
	// std::cout << trackinfo->hits.size() << std::endl;
	...
	// Float_t **HitPosition = new Float_t*[total_channels];
		...
		{
			fHitPosition[i][0] = (Float_t)trackinfo->hits[i].X;
			fHitPosition[i][1] = (Float_t)trackinfo->hits[i].Y;
			fHitPosition[i][2] = (Float_t)trackinfo->hits[i].Z;
			fCellSizes[i][0] = 15.;
			fCellSizes[i][1] = 15.;
			fCellSizes[i][2] = 12.;
		}
	...
	AllocData();
		...
		for(Int_t i = 0; i < fN_Hits; i++)
		{
			fCharge_Hits[i] = trackinfo->hits[i].charge;
...
void MuonTracker::AllocData()
{
	...
	fScintThickness = new Float_t[fN_Hits];
...
void MuonTracker::GetCalibratedCharge(TrackInfo* trackinfo)
{
	for (int i = 0; i < fN_Hits; i++)
	{
		trackinfo->hits[i].charge = fChargeStraightened[i];
	}
}
...
{
	Float_t straight_scint_path = 2.4; //in cm
	Float_t scint_fraction = 0.2;
	...
	for(Int_t i = 0; i < fN_Hits; i++)
	{
		...
		if(intersection_counter == 2)
		{
			...
			Double_t scint_length = scint_fraction*sqrt(SubstancePathSqMax); //in cm
			if(scint_length > 1e-1) //to cross at least one mm of scintilator
			    fChargeStraightened[i] = straight_scint_path*(fCharge_Hits[i]*fCharge_Calibration[i]/scint_length); //in adc channels

			fScintThickness[i] = scint_length;
			...
			if(fIsDebug)
			{
				...
				for(Int_t k = 0; k < intersection_counter; k++)
				{
				    ...
				    for(Int_t j = 0; j < fDimensions; j++) printf("%.0f ", IntersectVector[k][j]);
				    printf("\n");
				}
				printf("Scint thickss: %.2f ", fScintThickness[i]);
...
Float_t *MuonTracker::GetScintThickness()
{
	return fScintThickness;
}
